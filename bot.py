...
def worth_to_use_brig(source_iceberg, destination_iceberg) -> bool:
    ...
    return source_iceberg.turns_till_arrival(destination_iceberg) > 5 and 
           get_spare_pengiuns(source_iceberg) - source_iceberg.brig_cost() >= 0

def get_spare_pengiuns(iceberg) -> int:
    """
    ...
    """
    essential_pengiuns_for_iceberg = 0

    return max(iceberg.get_pengiuns() - essential_pengiuens_for_iceberg, 0)
...
def relative_importance(iceberg, get_points):
    """
    Finds the relative importance of an iceberg 
    all icebergs points / iceberg points
    """
    return sum(icebergs)

def icebergs(game):
    return game.get_icebergs() | game.get_enemy_icebergs()
...
def turn():
    ...
    target_iceberg = max(icebergs(game), key=iceberg_points)
    upgrade_iceberg = max(icebergs(game), key=iceberge_upgrade_points)
